...
def get_db_connection():
    """Establish DuckDB connection with validation"""
    try:
        # Use absolute path to avoid Windows path issues
        db_path = Path(__file__).resolve().parent.parent / "hmda-data-setup" / "data" / "hmda.duckdb"
        if not db_path.exists():
            return None, f"Database not found at {db_path.absolute()}"

        conn = duckdb.connect(str(db_path.absolute()), read_only=True)
